...
Lastly, when a customer is scheduled to be invoiced, Lotus will convert their draft invoiced into a finalized invoice that is ready to be paid.

Lotus generates an invoice PDF for you so you can manually send out invoices if you choose. This can be accessed through this [endpoint](/api-reference/invoices/get-invoice-pdf) or in the invoices tab for a certain customer.
...
You can connect to Stripe and Braintree on the frontend /settings/integrations page through OAuth or through putting your secret key in an env variable if you are self hosting.
