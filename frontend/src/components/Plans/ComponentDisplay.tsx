    ...
    case "per_unit":
      return (
        <span>
          ...
          {record.metric_units_per_batch && record.metric_units_per_batch > 1
            ? "s "
            : " "}
...
export const ComponentDisplay: FC<{
  ...
  <Row gutter={[12, 12]} className="overflow-y-auto max-h-[400px]">
    {componentsData.map((component) => (
      <Col span="24" key={component.metric_id}>
        <Paper border className="items-stretch">
          <Descriptions
            title={component?.metric_id}
            className="text-[20px]"
            column={2}
            extra={[
              <Button
              ...
              <Button
                ...
                danger
                onClick={() =>
                  deleteComponent(component.id as unknown as string)
                }
              />,
            ]}
          />
